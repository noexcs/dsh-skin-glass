...
└─ 毛玻璃 chrome（data-plugin-css 样式标签，官方支持的插件 CSS 注入）：
     ...
     运行时识别器给每个玻璃面打 data-dsh-glass-surface → 逐面 backdrop-filter
     （大浮层额外走 SVG feDisplacementMap 折射，仅 Chromium）
...
- **`position: fixed` 的元素会剥掉其所有玻璃祖先并永久拉黑**：`backdrop-filter` 会让元素
  成为其 fixed 后代的**包含块**。产品的设置弹窗是内联渲染的 `position: fixed; inset: 0`
  div，**就在侧边栏里面**（无 portal），所以侧边栏一旦成为玻璃面，`inset: 0` 的参照系就
  从视口塌缩成侧边栏，800px 的面板被 flex 压成侧边栏宽度。剥掉后视觉损失几乎为零：
  侧边栏贴着屏幕边缘，背后本来就只有已模糊的壁纸。
  剥离时会返回最上层那个被剥掉的祖先并重扫其子树——它们此前因为「在玻璃里」被跳过了。
- **跳过铺满视口的面**：它背后只有壁纸（本来就模糊过了），是全应用最贵的一次 backdrop
  却没有任何视觉收益，而且会成为其内部 fixed 元素的包含块。跳过但**继续下探**。
...
- `backdrop-filter` 会让元素成为 fixed 定位的包含块。逐面 backdrop-filter 把这个副作用带
  了回来，识别器用两条规则兜住：跳过铺满视口的面，以及剥掉任何 fixed 元素的玻璃祖先。
  **这条真的会咬人**——设置面板曾因此塌成侧边栏宽度，现在有专门的回归测试守着。
...
node scripts/check-contrast.mjs # 9100 组正文对比度断言 + 全令牌结构断言
node scripts/check-surfaces.mjs # 33 条玻璃面识别断言（含「绝不嵌套」「弹窗不塌缩」「异色不堆叠」）
...
- `backdrop-filter` 会让元素成为其内部 fixed 后代的包含块。铺满视口的面已被显式跳过，
  产品的 portal 弹层又挂在 `#root` 之外，所以实际影响面很小。
