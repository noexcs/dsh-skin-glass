...
两者现在在默认通透度下都落在 **0.58**——正是命令面板原本的观感（`FLOAT_TINT_FLOOR`
只在通透度拉满的那端才介入，见 src/color.cjs）。嵌套填充保持完整缩放（它们的职责是防
叠加，栈底下已有可读性下限），浮层自己的填充则不得低于 0.55 的地板。曾想把地板抬到
...
2. **body 的 style 属性写入触发全量重扫**（同样防抖——产品一次批量写几十个令牌，不防抖
   会连跑几十次 strip+重扫）。两个通道：令牌层由 ThemePresenter 异步写成 body 行内样式，
   走查可能跑在它落地之前；明暗主题切换会重写全部令牌，不重扫的话所有行内染色停留在
   旧模式的颜色。这里全量是对的——令牌变了，染色本来就都要重算。
