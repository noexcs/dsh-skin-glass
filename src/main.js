...
function apply(ctx) {
  ...
  ctx.effect(() => {
    ...
    const flushPending = () => {
      timer = null;
      const retag = pendingRetag;
      pendingRetag = false;
      const nodes = pending;
      pending = new Set();
      if (retag) scanner.retag();
    ...
    const observer = new MutationObserver((records) => {
      for (const record of records) {
        ...
        // styles on body — dozens of setProperty calls in one batch, so the
        // retag rides the same debounce. A walk that ran before the tokens
        // landed tags nothing, and a light/dark switch rewrites every
        // token, which would leave the inline tints in the old mode's
        // colours — both are fixed by a full re-walk.
